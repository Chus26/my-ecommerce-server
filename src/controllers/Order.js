...
// // ===== FILE: order.controller.js (Cập nhật getAdminOrders) =====
...
// //   try {
// //     // === SỬA ĐỔI: Thêm successfulOrders và tối ưu tính toán ===
// //     const [totalUsers, totalOrders, successfulOrders, earnings, balanceOrders] = await Promise.all([
// //       User.find({ role: "user" }).countDocuments(),
// //       Order.countDocuments({}), // Tổng số đơn dùng countDocuments
// //       Order.countDocuments({ paymentStatus: "Paid", deliveryStatus: "Delivered" }), // <-- Đếm đơn thành công
...
// //     // === Thêm successfulOrders vào response ===
// //     res.status(200).json({
// //       totalUsers,
// //       totalOrders,
// //       successfulOrders,
...
// // ===== THAY THẾ TOÀN BỘ HÀM NÀY TRONG: order.controller.js =====

// exports.getAdminOrders = async (req, res, next) => {
//   try {
//     // === (Phần thống kê của bạn giữ nguyên) ===
...
// // exports.getAdminOrders = async (req, res, next) => {
// //   try {
// //     //Lấy tổng số user, tổng số đơn hàng, tất cả đơn hàng
// //     const [totalUsers, totalOrders, orders] = await Promise.all([
// //       User.find({ role: "user" }).countDocuments(),
// //       Order.find({}).countDocuments(),
// //       // Nên populate nếu cần, nhưng ở đây chỉ tính tiền
// //       Order.find({}),
// //     ]);

// // // 1) TÍNH DOANH THU TỔNG: chỉ cộng đơn đủ điều kiện
// // // const earnings = orders.reduce(... // <-- BỎ cách cũ
...
// //     // Khoảng THÁNG HIỆN TẠI: dùng $gte start, $lt start của tháng kế tiếp để tránh lỗi ranh giới ngày cuối
// //     const startDateCurrentMonth = new Date(currentYear, currentMonth - 1, 1);
// //     const startDateNextMonth = new Date(currentYear, currentMonth, 1);

// // // 2) Doanh thu THÁNG HIỆN TẠI: chỉ đơn Paid + Delivered
// //     const ordersInMonth = await Order.find({
// //       createdAt: { $gte: startDateCurrentMonth, $lt: startDateNextMonth },
// //       paymentStatus: "Paid",
// //       deliveryStatus: "Delivered",
// //     });
// //     const totalCurrentMonthRevenue = ordersInMonth.reduce(
// //       (acc, curr) => acc + (Number(curr.totalPrice) || 0),
// //       0
// //     );

// //     // 3) TỔNG DOANH THU & TRUNG BÌNH THEO THÁNG (trong năm hiện tại)
// // //    chỉ cộng tháng có đơn Paid + Delivered
// //       let totalRevenue = 0;
// //       let monthsWithRevenue = 0;

// //       for (let month = 1; month <= currentMonth; month++) {
// //         const startDate = new Date(currentYear, month - 1, 1);
// //         const startOfNext = new Date(currentYear, month, 1);

// //         const ordersOfMonth = await Order.find({
// //           createdAt: { $gte: startDate, $lt: startOfNext },
// //           paymentStatus: "Paid",
// //           deliveryStatus: "Delivered",
// //         });

// //         const totalMonthRevenue = ordersOfMonth.reduce(
// //           (total, current) => total + (Number(current.totalPrice) || 0),
// //           0
// //         );

// //         totalRevenue += totalMonthRevenue;
// //         if (totalMonthRevenue > 0) monthsWithRevenue++;
// //       }

// //       const balance = monthsWithRevenue > 0 ? totalRevenue / monthsWithRevenue : 0;
...
// // Cập nhật trạng thái đơn (Admin): PATCH /api/orders/admin/:orderId/status
...
// //     await order.save(); // enum sẽ đảm bảo hợp lệ
// //     // 🔔 Emit realtime khi admin cập nhật trạng thái
// //     io.getIO().emit("order", {
...
// //     // 3) Cập nhật trạng thái đơn + emit
// //     order.deliveryStatus = "Canceled";
// //     if (order.paymentStatus === "Paid") order.paymentStatus = "Refunded";
// //     await order.save();

// //     io.getIO().emit("order", { action: "USER_CANCELED", orderId: order._id, order });
// //     return res.status(200).json({ message: "Đã hủy đơn hàng và hoàn kho.", order });
// //   } catch (error) {
// //     const err = new Error(error);
// //     err.httpStatus = 500;
// //     next(err);
...
// //           lineRevenue: { $multiply: [{ $toDouble: "$items.quantity" }, { $toDouble: "$items.product.price" }] }
// //         }
// //       },
// //       { $group: {
// //           _id: { id: "$productId", name: "$productName" },
// //           total: { $sum: "$lineRevenue" }
// //         }
// //       },
// //       { $project: { _id: 0, productId: "$_id.id", productName: "$_id.name", total: 1 } },
// //       { $sort: { total: -1 } },
// //       { $limit: limit }
// //     ]);
