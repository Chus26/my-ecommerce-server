...
// const User = require("../models/User"); // (Vì hàm mới có dùng User.findById)
// // ===================================
...
//   const { code, name, category, shortDescription, longDescription, price, stock } = req.body;
...
//   // helper nội bộ (không đổi cấu trúc file)
...
//     if (val == null) return [];
//     // có thể là "0", ["0","2"], hoặc nhiều key replaceIndexes[]
...
//         // Upload tất cả ảnh được gửi
//         const uploaded = await uploadMany(req.files, "products");
//         // Thay từng slot theo thứ tự FE gửi
...
//           // Xoá ảnh cũ ở slot này (nếu có)
//           const oldPid = p[pidKey];
//           if (oldPid) {
//             try { await destroy(oldPid); } catch (e) { /* bỏ qua lỗi xoá */ }
//           }

//           // Gán ảnh mới cho đúng key
//           p[urlKey] = fileRes.url;
//           p[pidKey] = fileRes.public_id;
//         }
//       } else if (req.files.length === 4) {
//         // Không có replaceIndexes -> giữ nguyên logic cũ: thay cả 4 ảnh
...
//       } else {
//         // Có file nhưng không đủ 4 và cũng không gửi replaceIndexes -> báo lỗi rõ ràng
//         return res.status(400).json({
...
// //     }
// //     if (!product.img4.includes("firebase")) {
...
// // === THÊM HÀM MỚI: Thêm đánh giá sản phẩm ===
...
//       return res
//         .status(400)
//         .json({ message: "Vui lòng cung cấp xếp hạng (rating) từ 1 đến 5." });
//     }
//     if (!comment || comment.trim() === "") {
//       return res
//         .status(400)
//         .json({ message: "Vui lòng nhập nội dung đánh giá." });
//     }
...
//         message:
//           "Bạn phải mua và nhận hàng thành công mới có thể đánh giá.",
//       });
//     }
...
//       // Bộ lọc an toàn để tránh lỗi 500
